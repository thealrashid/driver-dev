...
void workqueue_fn(struct work_struct *my_work);
...
static irqreturn_t irq_handler(int irq,void *dev_id) {
	pr_info("Shared IRQ: Interrupt Occurred on IRQ %d\n", irq);
	
	/* Allocating work to workqueue */
...
static int __init my_char_module_init(void) {
	...
	/* Registering the IRQ */
	if (request_irq(IRQ_NO, irq_handler, IRQF_SHARED, "My_char_dev", (void *)(irq_handler))) {
		pr_err("my_device: cannot register IRQ ");
		goto r_irq;
	}
	
	/* Create workqueue */
	my_workqueue = alloc_workqueue("my_workqueue", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	if (!my_workqueue) {
		pr_err("Cannot create my workqueue\n");
		goto r_irq;
	}
