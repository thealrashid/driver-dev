...
static struct task_struct *my_thread_1;
static struct task_struct *my_thread_2;
...
int my_thread_function_1(void *arg);
int my_thread_function_2(void *arg);

int my_thread_function_1(void *arg) {
	while (!kthread_should_stop()) {
		mutex_lock(&my_mutex);
		my_global_variable++;
		pr_info("My kernel thread 1 running: %lu\n", my_global_variable);
		mutex_unlock(&my_mutex);
		ssleep(1);
	}
	
	pr_info("Kernel thread 1 stopping\n");
...
int my_thread_function_2(void *arg) {
	while (!kthread_should_stop()) {
		mutex_lock(&my_mutex);
		my_global_variable++;
		pr_info("My kernel thread 2 running: %lu\n", my_global_variable);
		mutex_unlock(&my_mutex);
		ssleep(1);
	}
	
	pr_info("Kernel thread 2 stopping\n");
...
static int __init my_char_module_init(void) {
	...
	my_thread_1 = kthread_create(my_thread_function_1, NULL, "my_thread_1");
	if (my_thread_1) {
		wake_up_process(my_thread_1);
	} else {
		pr_err("Cannot create my_thread_1\n");
		goto r_device;
	}
	
	my_thread_2 = kthread_create(my_thread_function_2, NULL, "my_thread_2");
	if (my_thread_2) {
		wake_up_process(my_thread_2);
	} else {
		pr_err("Cannot create my_thread_2\n");
		goto r_device;
	}
...
static void __exit my_char_module_exit(void) {
	kthread_stop(my_thread_1);
	kthread_stop(my_thread_2);
